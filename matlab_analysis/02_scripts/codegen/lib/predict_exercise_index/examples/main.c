...
void main_predict_exercise_index(void)
{
  double b_dv[32];
  int idx;
  /* Initialize function 'predict_exercise_index' input arguments. */
  /* Initialize function input argument 'features'. */
  /* Call the entry-point 'predict_exercise_index'. */
  argInit_1x32_real_T(b_dv);
  idx = predict_exercise_index(b_dv);
}
